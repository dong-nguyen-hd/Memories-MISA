    ...
    public class FeeService : BaseService<FeeResource, CreateFeeResource, UpdateFeeResource, Fee>, IFeeService
    {
        ...
        public override async Task<BaseResponse<FeeResource>> GetByIdAsync(Guid id)
        {
            try
            {
                ...
                if (fee is null)
                    return new BaseResponse<FeeResource>("Đối tượng không tồn tại.");
            ...
            {
                return new BaseResponse<FeeResource>($"Lỗi khi thực hiện lấy dữ liệu: {ex.Message}");
            }
        }
        ...
        public async Task<BaseResponse<IEnumerable<FeeResource>>> GetAllPaginationAsync(SearchFeeResource searchResource)
        {
            ...
            {
                return new BaseResponse<IEnumerable<FeeResource>>($"Lỗi khi thực hiện lấy dữ liệu: {ex.Message}");
            }
        ...
        public async Task<DeleteResponse<IEnumerable<DetailDelete>>> DeleteManyAsync(DeleteResource deleteResource)
        {
            ...
            {
                return new DeleteResponse<IEnumerable<DetailDelete>>($"Lỗi khi thực hiện xoá đối tượng: {ex.Message}");
            }
        }
        ...
        public override async Task<BaseResponse<FeeResource>> InsertAsync(CreateFeeResource createFeeResource)
        {
            try
            {
                ...
                if (insertResponse <= 0)
                    return new BaseResponse<FeeResource>("Lỗi khi thực hiện thêm mới đối tượng.");
                ...
                return insertResponse > 0 ? new BaseResponse<FeeResource>(feeResource)
                    : new BaseResponse<FeeResource>("Lỗi khi thực hiện thêm mới đối tượng.");
            }
            catch (Exception ex)
            {
                return new BaseResponse<FeeResource>($"Lỗi khi thực hiện thêm mới đối tượng: {ex.Message}");
            }
        }
        ...
        public override async Task<BaseResponse<FeeResource>> UpdateAsync(Guid id, UpdateFeeResource updateFeeResource)
        {
            try
            {
                ...
                var fee = await _feeRepository.GetByIdAsync(id);
                if (fee is null)
                    return new BaseResponse<FeeResource>("FeeNameId không tồn tại trong CSDL.");
                ...
                return tempData > 0 ? new BaseResponse<FeeResource>(feeResource)
                    : new BaseResponse<FeeResource>("Lỗi khi thực hiện cập nhật đối tượng.");
            }
            catch (Exception ex)
            {
                return new BaseResponse<FeeResource>($"Lỗi khi thực hiện cập nhật đối tượng: {ex.Message}");
            }
        }
        ...
        private async Task<(bool isSuccess, List<string> message)> ValidateValueResource<T>(T feeResouce, Guid? isUpdate = null)
        {
            ...
            try
            {
                ...
                {
                    if (!isValidFeeName)
                        messages.Add("Giá trị thuộc tính FeeName đã tồn tại trong CSDL.");
                    if (unitFeeTask.Result is null)
                        messages.Add("Giá trị thuộc tính UnitFeeId không tồn tại trong CSDL.");
                    if (feeRangeTask.Result is null)
                        messages.Add("Giá trị thuộc tính FeeRangeId không tồn tại trong CSDL.");
                ...
                if (feeGroupId != null)
                {
                    ...
                    {
                        messages.Add("FeeGroupId không tồn tại trong CSDL.");
                        return (false, messages);
                    }
                }
